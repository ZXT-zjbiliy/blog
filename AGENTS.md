...
- Build-time index is **split into two prerendered endpoints** (both `export const prerender = true`), sharing build logic in `src/lib/search-build.ts` (`buildEntries()` / `buildTokens()`):
  ...
  - `src/pages/search-tokens.json.ts` → `/blog/search-tokens.json`: **heavy**, `{ tokens, texts }`. `tokens` is the full-text inverted index of all note bodies (null-prototype object via `Object.create(null)` — avoids prototype-chain collisions with words like "constructor"). `texts` is an array (aligned with `entries[]` index) of plain-text note bodies (`stripMarkdown()` in `search-build.ts`) used to extract highlighted result snippets; projects/topics store `""`. Fetched **only** when the user clicks "Full text / 全文".
...
- Result rendering: matched keywords in title/description are wrapped in `<mark>` via a client-side `highlight()` (HTML-escaped first). In full-text mode a highlighted body snippet (`makeSnippet()`, ~40-char window around the first hit) is appended to matching notes. TOPIC results are visually distinct — filled accent badge (`.topic-badge`) + accent left rail + tinted row (`.result-item.topic a`).
- The tag panel is a single-row auto-scrolling marquee (`.tag-marquee` / `.tag-track`, two duplicated `<ul>` copies for a seamless loop, pause-on-hover, `prefers-reduced-motion` → manual horizontal scroll, and `@media (hover: none)` → static + horizontally scrollable so touch users can tap tags; the JS-created `.tag-track`/`ul` need `:global()` CSS since Astro scoped styles only target template markup). Selected filters render as single rounded-rectangle chips (`--radius`) with an inline `×` in the search field.
- Keyboard & a11y: the input is a `role="combobox"` with `aria-expanded`/`aria-controls`/`aria-activedescendant`; results are a `role="listbox"` of `role="option"` rows. ↑/↓ move a `.kb-active` highlight (wraps, scrolls into view), Enter opens it, Escape closes. A `data-search-clear` `×` button (shown when there's a query or active tags) resets query + all tag filters.
- **Gotcha — Astro scoped CSS never matches JS-created nodes.** Every element `SearchBar.astro`'s `<script>` builds at runtime (result `li`/`a`/`.result-badge`/`.result-title`/`.result-desc`/`.result-snippet`, `.chip`, tag `.tag` pills, `.tag-track`) does NOT get the `[data-astro-cid]` attribute, so a plain scoped rule compiles to `.foo[cid]` and silently fails. All such rules live in a **`<style is:global>` block scoped under `.search-wrap`** (the marquee `.tag-track` set uses inline `:global(...)` in the scoped block instead — same effect). When adding a style for any dynamically-created search element, put it in the global block, not the scoped one.
