...
- The tag panel is a single-row auto-scrolling marquee (`.tag-marquee` / `.tag-track`, two duplicated `<ul>` copies for a seamless loop, pause-on-hover, `prefers-reduced-motion` → manual horizontal scroll; the JS-created `.tag-track`/`ul` need `:global()` CSS since Astro scoped styles only target template markup). Selected filters render as single rounded-rectangle chips (`--radius`) with an inline `×` in the search field.
